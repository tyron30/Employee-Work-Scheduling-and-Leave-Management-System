...
// Fetch only pending leave data from the database
$sql = "SELECT 
        ...
        WHERE tblleaves.Status = 0"; // Filter for pending status
...
header("Content-Disposition: attachment; filename=pending-leave-report.doc");
...
echo "</head>";
echo "<body>";
echo "<h2>Pending Employee Leave Report</h2>";
echo "<table>";
echo "<tr>
        <th>Employee ID</th>
        <th>Full Name</th>
...
if ($query->rowCount() > 0) {
    foreach ($results as $result) {
        ...
        $status = ($result->Status == 0) ? "Pending" : "Unknown";
        echo "<tr>
                <td>{$result->EmpId}</td>
